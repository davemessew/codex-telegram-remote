...
- The default `appServer` backend routes prompts into existing GUI threads. It uses the selected project's latest GUI thread unless the user chooses another one with `/thread`.
- `/thread` opens a tappable thread picker for the selected project and stores the selected GUI thread per chat and project.
...
- Completion messages include the exact final answer under `Details:` when `sendFullFinalAnswer` is enabled. A separate summary is shown when the backend provides one.
